...
> **通知先は private チャンネルか、自分あてのDMにしてください。**
> 通知にはメールの件名・差出人・要約が載ります。public チャンネルに流すと、
> ワークスペースの全員が取引先の用件を読める状態になります。
>
> また、このURLを知っている人は誰でもそのチャンネルに投稿できます。**パスワードと同じ扱いで、外部に共有しないでください。**
...
- 分類の内訳が実際の受信状況と合っているか
- 保留が多すぎないか（全体の2割を超えるようなら担当エンジニアに相談してください）

> **実行ログには件名も差出人も出ません。** 表示されるのはメッセージIDと分類だけです。
> **公開リポジトリの Actions 実行ログは誰でも読めるため**、取引先の名前や用件が残らないようにしてあります。
> 個別のメールを確認したいときは、Gmailで実物を開いてください。
...
| `TRIAGE_LOG_CONTENT` | Variable | – | `false` | 実行ログに件名・差出人を出すか。**公開リポジトリでは false のままにする** |
