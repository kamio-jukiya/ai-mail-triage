...
技術選定より、**運用に乗るかどうか**を優先して設計しています。以下の8点がこのツールの中身です。
...
### 7. イベントで取れない情報は、状態の経過で補う

メールはイベントです。**「何も起きていない」ことは、イベントとしては絶対に届きません。**

| 情報 | 取り方 |
|---|---|
| 届いたもの | **イベント**としてメールから取る |
| 何も来ていないこと | **状態**として経過時間から生成する |

イベント駆動だけで設計すると、無音を検知できない仕組みができあがります。「届いたメールを分類する」と「保留のまま3日誰も触っていないことに気づく」は、別の取り方をしないと両立しません。

> 現状のこのツールは前者しか実装していません。後者は設計上の指針として記載し、必要になった時点で足します。処理済みIDという状態は既に持っているので、足す場所は決まっています。

### 8. 能動的リスクと受動的リスクを区別する

各サービスの管理画面をスクレイピングして状態を取る案も検討しましたが、採用しませんでした。

依存先が同じでも、リスクの種類が違います。

- 自動ログイン ＝「**こちらの操作が原因で**アカウントが止まる」能動的リスク
- 公式APIの読み取り ＝「向こうが止まったら止まる」受動的リスク。**自分の行動で増減しない**

壊れ方も違います。DOMセレクタはクラス名が1文字変われば黙って壊れます。メール文面はテンプレートが変わっても、AI分類なら意味で吸収できます。**「文面は安定している」とは主張しません。壊れ方の性質が違う**という話です。
