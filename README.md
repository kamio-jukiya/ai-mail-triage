...
### イベントで取れない情報は、状態の経過で補う

メールはイベントです。**「何も起きていない」ことは、イベントとしては絶対に届きません。** 受信を起点にした処理だけを組むと、無音を検知できない仕組みができあがります。

| 知りたいこと | 取り方 |
|---|---|
| 届いたもの | **イベント**として、受信メールから取る |
| 何も来ていないこと | **状態**として、記録の経過時間から生成する |

このツールが持つ状態は処理済みIDの記録です。イベント（受信）だけでなく状態（記録とその経過）を持っているので、「保留にしたまま3日誰も触っていないメール」のような、届かない情報をこちら側から作れます。追跡を名乗るなら、この2つが揃って初めて成立します。

### 入口をメールにした理由

各サービスの管理画面にログインして状態を読み取る案も検討しましたが、採りませんでした。

- **壊れやすい層に乗ることになる。** セッション切れ、2要素認証、DOM変更、CAPTCHA。自動化の中で最も壊れやすい部分に依存する構成になります
- **壊れ方の性質が違う。** DOMセレクタはクラス名が1文字変われば黙って壊れます。メールはテンプレートが変わっても、意味で分類している限り吸収できます。文面が安定しているという主張ではなく、壊れ方の質が違うという話です
- **リスクの向きが違う。** 自動ログインは「こちらの操作が原因で止まる」能動的なリスク、公式APIの読み取りは「向こうが止まったら止まる」受動的なリスクです。後者は自分の行動で増減しません
...
- 400系エラーを再試行しないこと
- 指示を含むメールで分類が乗っ取られないこと、下書きが作られないこと
...
- **滞留の検知は未実装** — 状態（処理済みIDの記録）は持っていますが、「保留のまま何日経ったか」を通知する処理はまだ書いていません。上の設計判断で述べた無音の検知は、現時点では入口を用意した段階です。
